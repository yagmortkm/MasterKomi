...
EVENT_DIRECTION_VALUES = {v for v, _ in EVENT_DIRECTIONS}


@app.context_processor
def direct_labs():
    return {"direction_labels": dict(EVENT_DIRECTIONS)}
...
def index():
    ...
    return render_template(
        "index.html", title="Главная страница", username=user
    )
...
class MasterclassForm(FlaskForm):
    ...
    direction = SelectField(
        "Направление", choices=EVENT_DIRECTIONS, validators=[DataRequired()]
    )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = (
            db_sess.query(User)
            .filter((User.name == login_value) | (User.email == login_value))
            .first()
        )
        if (
            user
...
def profile():
    ...
    my_signups = (
        db_sess.query(EventSignup)
        .filter(EventSignup.user_id == user.id)
        .order_by(EventSignup.created_date )
        .all()
    )
    cancel_signup_forms = {
        s.event_id: CancelFrom(prefix=f"c{s.event_id}") for s in my_signups
    }
    delete_event_forms = {
        e.id: DeleteEventForm(prefix=f"d{e.id}") for e in my_events
    }
    return render_template(
        "profile.html",
        title="Профиль",
        ...
        cancel_signup_forms=cancel_signup_forms,
...
def masterclasses():
    db_sess = db_session.create_session()
    q = db_sess.query(Event).order_by(Event.created_date.desc())
    direction = request.args.get("direction", type=str)
    filter_direction = (
        direction if direction in EVENT_DIRECTION_VALUES else None
    )
    if filter_direction:
        q = q.filter(Event.direction == filter_direction)
    items = q.all()
    ...
    if user_id:
        signed_ids = {
            r[0]
            for r in db_sess.query(EventSignup.event_id)
            .filter(EventSignup.user_id == user_id)
            .all()
        }
    signup_forms_by_id = {
        e.id: EventSignupForm(prefix=f"e{e.id}") for e in items
    }
    return render_template(
        "masterclasses.html",
        title="Мастер-классы",
        masterclasses=items,
        user_id=user_id,
        signed_event_ids=signed_ids,
        signup_forms_by_id=signup_forms_by_id,
...
def signup_event(event_id):
    ...
    if (
        db_sess.query(EventSignup)
        .filter_by(user_id=uid, event_id=event_id)
        .first()
    ):
...
def delete_event(event_id):
    if not session.get("user_id"):
        return redirect(url_for("login"))
    form = DeleteEventForm(prefix=f"d{event_id}")
    if not form.validate_on_submit():
...
def cancel_signup(event_id):
    ...
    rec = (
        db_sess.query(EventSignup)
        .filter_by(user_id=session["user_id"], event_id=event_id)
        .first()
    )
    if rec:
        db_sess.delete(rec)
    ...
    if form.validate_on_submit():
        try:
            ...
            return render_template(
                ...
                form=form,
                message="Укажите цену целым числом (0 если бесплатно)",
            )
        try:
            event_date = datetime.datetime.strptime(
                form.date.data.strip(), "%Y-%m-%d"
            )
        except ValueError:
            return render_template(
                "add_event.html",
        ...
        return redirect(url_for("masterclasses"))
    return render_template(
        "add_event.html", title="Новый мастер-класс", form=form
    )
